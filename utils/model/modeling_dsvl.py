...
def create_dsvl_model_and_transforms(
    ...
    lang_config = AutoConfig.from_pretrained(args.lm_model_name_or_path, trust_remote_code=True)
    ...
    if 'llama' in args.lm_model_name_or_path.lower():
        ...
        decoder_name = 'llama'
    elif 'baichuan' in args.lm_model_name_or_path.lower():
        if ds_config is not None and ds_config["zero_optimization"]["stage"] == 3:
            lang_decoder = AutoModelForCausalLM.from_pretrained(args.lm_model_name_or_path, config=lang_config, trust_remote_code=True)
        else:
            try:
                device = torch.device("cuda", args.local_rank)
            except:
                device = "auto"
            lang_decoder = AutoModelForCausalLM.from_pretrained(args.lm_model_name_or_path, config=lang_config, device_map=device, trust_remote_code=True)
        decoder_name = 'baichuan2'
    else:
...
class DeepSpeedViLModel(nn.Module):
    ...
    def _get_model_stat(self, model_name):   
        config_dic = {
            'baichuan2': ['model_max_length','num_hidden_layers'],
